...
def train_and_evaluate(
    ...
    # Load ds
    ds_train = tf.data.Dataset.load(f"{ds_path}", compression="GZIP")

    validation_dataset = ds_train.take(200)
    train_dataset = ds_train.skip(200)

    # Checkpoint path
    if ckpt_path is None:
        ckpt_path = f"/code/ckpt/{model.name}_{tstamp}"
    # Tensorboard path
    if tensorboard_path is None:
        tensorboard_path = f"/code/tensorboard/{model.name}_{tstamp}"

    # Apply min-max normalization
    model.set_min_max_scores(get_min_max_dict(train_dataset, model.min_max_scores_fields))
...
if __name__ == "__main__":
    ...
    model = models.DelayPredictor

    _reset_seeds()
    trained_model, evaluation = train_and_evaluate(
        "/code/data/data_train",
        model(),
