...
def _get_network_decomposition(sample: Sample) -> Tuple[dict, list]:
    ...
    for src, dst in filter(
        ...
        for local_flow_id in range(len(traffic_matrix[src, dst]["Flows"])):
            ...
            try:
                sending_duration = packet_timestamps[-1] - packet_timestamps[0]
            except:
                sending_duration = 0
            ...
            flows[flow_id] = {
                ...
                "ipg_var": np.var(ipg),
                "ipg_percentiles": np.nanpercentile(ipg, [10 * i for i in range(1, 10)]),
    ...
    sample = (
        {
            ...
            "sending_duration": np.expand_dims([flow["sending_duration"] for flow in ordered_flows], axis=1),
    ...
    signature = (
        {
            ...
            "flow_ipg_percentiles": tf.TensorSpec(shape=(None, 9), dtype=tf.float32),
            "sending_duration": tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
